class MenuPublishReadinessValidator < ActiveModel::Validator
  def validate(record)
    ...
    errors << 'Менее двух супов' unless less_than?(record, :soup, 2)
    errors << 'Менее двух салатов' unless less_than?(record, :salad, 2)
  ...
  def less_than?(record, dish_type, required_number)
    record.dishes.where(dish_type: dish_type).size < required_number
  end

  def exists_default?(record, dish_type)
    record.dishes.where(dish_type: dish_type).menu_dishes.exists?(default: true)
  end
